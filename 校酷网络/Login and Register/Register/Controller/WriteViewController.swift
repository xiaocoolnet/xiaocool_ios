...
//    下一步
    @IBAction func nextBtn(sender: AnyObject) {
        if (numTextField.text!.isEmpty){
            ...
            let vc : UIViewController = mainStoryboard.instantiateViewControllerWithIdentifier("SetpwView")
...
//    倒计时开始
    func timeDown(){
        let time1 = NSTimer.scheduledTimerWithTimeInterval(1.0, target: self, selector: #selector(WriteViewController.updateTime), userInfo: nil, repeats: true)
        timeNow = time1
    }
//    展现再次发送的按钮
